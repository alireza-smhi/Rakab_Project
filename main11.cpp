...
public:
    ...
    void CalculatePoints(vector <Player> &p) {
        ...
        for (int i = 0; i < pointsEarned.size(); i++) {
            ...
            for (int j = 0; j < purpleCards[i].size(); i++) {
    ...
    void war(vector <Player> &p) {
        ...
        while (!warEnd) {
            ...
            cout << endl;
            p[turn].choosing(yellowCards[turn], purpleCards[turn], passedPlayers);
            if (purpleCards[turn].size() != 0) {
            if (purpleCards[turn][purpleCards[turn].size()-1] -> getPriority() == 0) {
                purpleCards[turn][purpleCards[turn].size()-1] -> makeEffect(p[turn], yellowCards[turn], pointsEarned[turn]);
                purpleCards[turn].erase(purpleCards[turn].begin() + purpleCards[turn].size()-1);
            }
            }
