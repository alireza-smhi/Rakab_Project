...
class PurpleCard : public Card {
    ...
    public:
        ...
        virtual void makeEffect(class state &s, vector <Player> &p, int turn) = 0;
...
public:
    ...
    void doWhatPlayerSaid(string command, vector <Player> &p, int turn) {
        vector <YellowCard> yellowTemp = p[turn].getYellowCards();            //inja vector haye movaghaty gerefte shode ta betonim karti as daron vector ra hazf konim
        vector <PurpleCard*> purpleTemp = p[turn].getPurpleCards();
        for (int i = 0; i < yellowTemp.size(); i++) {
            if (command == yellowTemp[i].getName()) {
                ...
                p[turn].setYellowCards(yellowTemp);
                return;
            }
        }
        for (int i = 0; i < purpleTemp.size(); i++) {
            if (command == purpleTemp[i] -> getName()) {
                if (purpleTemp[i] -> getPriority() == 0) {
                    purpleTemp[i] -> makeEffect(*this, p, turn);
                    purpleTemp.erase(purpleTemp.begin() + i);
                }
                else if (purpleTemp[i] -> getName() == "bahaar") {
                    season = purpleTemp[i];
                }
                else {
